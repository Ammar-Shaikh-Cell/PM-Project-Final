...
async def startup_event():
    ...
    try:
        ...
        from app.tasks.seed_extruder_month_history import seed_extruder_month_history
        ...
        # Auto-seed one month of dummy extruder history if none exists yet
        try:
            logger.info("Checking if extruder month history seeding is needed...")
            await seed_extruder_month_history()
        except Exception as se:
            logger.error(f"Month history seeding failed (non-blocking): {se}")
    except Exception as e:
