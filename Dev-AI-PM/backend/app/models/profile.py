...
class Profile(Base):
    ...
    pressure_configs = relationship(
        "ProfilePressureConfig", back_populates="profile", cascade="all, delete-orphan"
    )
...
class ProfilePressureConfig(Base):
    """
    Per-profile melt pressure configuration (per material / optional batch/charge).

    This drives:
    - chart bands/colors for Pressure_bar
    - warning/critical/low alert thresholds
    - which emails are sent for warning/critical/start/stop
    """

    __tablename__ = "profile_pressure_configs"
    ...
    # Optional batch/charge identifier (if you want different ranges per charge)
    batch_id = Column(String(100), nullable=True, index=True)

    # Ranges (bar)
    cool_from = Column(Float, nullable=False, default=330.0)
    cool_to = Column(Float, nullable=False, default=360.0)
    medium_from = Column(Float, nullable=False, default=360.0)
    medium_to = Column(Float, nullable=False, default=380.0)
    hot_from = Column(Float, nullable=False, default=380.0)
    hot_to = Column(Float, nullable=False, default=395.0)
    critical_from = Column(Float, nullable=False, default=395.0)
    critical_to = Column(Float, nullable=False, default=410.0)

    # Alert thresholds (bar)
    warning_threshold = Column(Float, nullable=False, default=380.0)
    critical_warning_threshold = Column(Float, nullable=False, default=395.0)
    low_pressure_warning_threshold = Column(Float, nullable=False, default=340.0)

    # Email toggles
    send_email_on_warning = Column(Boolean, nullable=False, default=True)
    send_email_on_critical = Column(Boolean, nullable=False, default=True)
    send_email_on_production_start = Column(Boolean, nullable=False, default=False)
    send_email_on_production_stop = Column(Boolean, nullable=False, default=False)

    # Which config is used when multiple exist for the same profile/material
    is_active = Column(Boolean, nullable=False, default=True)

    created_at = Column(DateTime(timezone=True), default=datetime.utcnow)
    updated_at = Column(DateTime(timezone=True), default=datetime.utcnow)

    profile = relationship("Profile", back_populates="pressure_configs")
