...
class PressureConfigUpsert(BaseModel):
    batch_id: Optional[str] = None
    is_active: bool = True

    cool_from: float = 330.0
    cool_to: float = 360.0
    medium_from: float = 360.0
    medium_to: float = 380.0
    hot_from: float = 380.0
    hot_to: float = 395.0
    critical_from: float = 395.0
    critical_to: float = 410.0

    warning_threshold: float = 380.0
    critical_warning_threshold: float = 395.0
    low_pressure_warning_threshold: float = 340.0

    send_email_on_warning: bool = True
    send_email_on_critical: bool = True
    send_email_on_production_start: bool = False
    send_email_on_production_stop: bool = False
...
@router.get("/{profile_id}/pressure-config")
async def get_pressure_config(
    profile_id: UUID,
    batch_id: Optional[str] = Query(None, description="Optional batch/charge ID"),
    session: AsyncSession = Depends(get_session),
    current_user: User = Depends(get_current_user),
) -> Dict[str, Any]:
    """Get active melt-pressure configuration for a profile (optionally per batch)."""
    # Prefer exact (profile,batch,is_active), else fallback to (profile,NULL,is_active)
    q = (
        select(ProfilePressureConfig)
        .where(ProfilePressureConfig.profile_id == profile_id)
        .where(ProfilePressureConfig.is_active == True)
    )
    if batch_id is not None:
        q1 = q.where(ProfilePressureConfig.batch_id == batch_id)
        r1 = await session.execute(q1)
        cfg = r1.scalar_one_or_none()
        if cfg:
            return _pressure_cfg_to_dict(cfg)
    q2 = q.where(ProfilePressureConfig.batch_id.is_(None))
    r2 = await session.execute(q2)
    cfg2 = r2.scalar_one_or_none()
    if cfg2:
        return _pressure_cfg_to_dict(cfg2)
    # If none exists, return defaults (not persisted) so UI can show something
    return _pressure_cfg_to_dict(ProfilePressureConfig(profile_id=profile_id))


@router.put("/{profile_id}/pressure-config")
async def upsert_pressure_config(
    profile_id: UUID,
    payload: PressureConfigUpsert = Body(...),
    session: AsyncSession = Depends(get_session),
    current_user: User = Depends(require_engineer),
) -> Dict[str, Any]:
    """Create or update melt-pressure configuration for a profile (optionally per batch)."""
    # Ensure profile exists
    prof = (await session.execute(select(Profile).where(Profile.id == profile_id))).scalar_one_or_none()
    if not prof:
        raise HTTPException(status_code=404, detail="Profile not found")
    ...
    q = (
        select(ProfilePressureConfig)
        .where(ProfilePressureConfig.profile_id == profile_id)
        .where(ProfilePressureConfig.batch_id == payload.batch_id)
    )
    existing = (await session.execute(q)).scalar_one_or_none()

    if existing is None:
        cfg = ProfilePressureConfig(id=uuid4(), profile_id=profile_id, batch_id=payload.batch_id)
    else:
        cfg = existing
...
def _pressure_cfg_to_dict(cfg: ProfilePressureConfig) -> Dict[str, Any]:
    return {
        "id": str(getattr(cfg, "id", "")) if getattr(cfg, "id", None) else None,
        "profile_id": str(cfg.profile_id) if cfg.profile_id else None,
        "batch_id": cfg.batch_id,
        "is_active": bool(cfg.is_active) if cfg.is_active is not None else True,
        "ranges": {
            "cool": {"from": float(cfg.cool_from), "to": float(cfg.cool_to)},
            "medium": {"from": float(cfg.medium_from), "to": float(cfg.medium_to)},
            "hot": {"from": float(cfg.hot_from), "to": float(cfg.hot_to)},
            "critical": {"from": float(cfg.critical_from), "to": float(cfg.critical_to)},
        },
        "thresholds": {
            "warning": float(cfg.warning_threshold),
            "critical": float(cfg.critical_warning_threshold),
            "low": float(cfg.low_pressure_warning_threshold),
        },
        "emails": {
            "on_warning": bool(cfg.send_email_on_warning),
            "on_critical": bool(cfg.send_email_on_critical),
            "on_production_start": bool(cfg.send_email_on_production_start),
            "on_production_stop": bool(cfg.send_email_on_production_stop),
        },
    }
