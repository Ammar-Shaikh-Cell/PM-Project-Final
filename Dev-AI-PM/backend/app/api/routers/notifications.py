...
class TestPressureEmailRequest(BaseModel):
    """Trigger the same pressure notification emails sent by the MSSQL poller (for testing)."""
    pressure_bar: float
    type: str = "low"  # "low" = pressure under 340 bar, "high" = pressure over 380 bar
...
@router.post("/test-pressure-email")
async def trigger_test_pressure_email(
    payload: TestPressureEmailRequest,
    current_user: User = Depends(require_engineer),
):
    """
    Send the same extruder pressure notification email that the MSSQL poller would send.
    Use this to verify that pressure emails reach your recipients (SMTP + Email Recipients).
    Does not require the poller to be running.
    """
    if not notification_service.email_configured():
        return JSONResponse(
            status_code=status.HTTP_400_BAD_REQUEST,
            content={
                "ok": False,
                "error": "Email SMTP credentials not configured. Set EMAIL_SMTP_* in backend/.env",
            },
        )
    try:
        if payload.type == "high":
            await notification_service.send_extruder_high_pressure_email(payload.pressure_bar)
            return {
                "ok": True,
                "message": f"High-pressure test email sent (pressure={payload.pressure_bar} bar). Check recipient inboxes.",
            }
        else:
            await notification_service.send_extruder_low_pressure_email(payload.pressure_bar)
            return {
                "ok": True,
                "message": f"Low-pressure test email sent (pressure={payload.pressure_bar} bar). Check recipient inboxes.",
            }
    except Exception as e:
        logger.error(f"Test pressure email failed: {e}", exc_info=True)
        return JSONResponse(
            status_code=status.HTTP_502_BAD_GATEWAY,
