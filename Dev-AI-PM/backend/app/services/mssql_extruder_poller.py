...
class MSSQLExtruderPoller:
    def __init__(
        ...
        *,
        ...
        # Runtime pressure alert & state tracking
        self._last_pressure_value: Optional[float] = None
        self._last_pressure_high_email_at: Optional[datetime] = None
        self._last_pressure_low_email_at: Optional[datetime] = None
        self._pressure_email_cooldown_seconds: int = 1800  # 30 minutes cooldown
        self._extruder_running: bool = False
        self._last_start_email_at: Optional[datetime] = None
        self._last_shutdown_email_at: Optional[datetime] = None
        self._start_stop_email_cooldown_seconds: int = 600  # 10 minutes cooldown
        ...
        while not self._stop.is_set():
            try:
                ...
                if new_rows:
                    ...
                    # ---------------- Pressure-based alerts & start/stop detection ----------------
                    try:
                        from app.services import notification_service

                        current_pressure = readings.get("pressure")
                        current_rpm = readings.get("rpm")
                        if current_pressure is not None:
                            prev_pressure = self._last_pressure_value
                            self._last_pressure_value = float(current_pressure)

                            # Gate high/low pressure emails on PRODUCTION state.
                            # Use the same practical thresholds as machine-state defaults:
                            # rpm_prod ~= 10, p_prod ~= 5.
                            is_in_production = (
                                (current_rpm is not None and float(current_rpm) >= 10.0)
                                and float(current_pressure) >= 5.0
                            )

                            # High pressure > 380 bar
                            if is_in_production and current_pressure > 380.0:
                                should_send = (
                                    self._last_pressure_high_email_at is None
                                    or (datetime.utcnow() - self._last_pressure_high_email_at).total_seconds()
                                    > self._pressure_email_cooldown_seconds
                                )
                                if should_send:
                                    await notification_service.send_extruder_high_pressure_email(current_pressure)
                                    self._last_pressure_high_email_at = datetime.utcnow()

                            # Low pressure < 340 bar
                            if is_in_production and current_pressure < 340.0:
                                should_send = (
                                    self._last_pressure_low_email_at is None
                                    or (datetime.utcnow() - self._last_pressure_low_email_at).total_seconds()
                                    > self._pressure_email_cooldown_seconds
                                )
                                if should_send:
                                    await notification_service.send_extruder_low_pressure_email(current_pressure)
                                    self._last_pressure_low_email_at = datetime.utcnow()

                            # Start detection: pressure < 50 -> > 300
                            if prev_pressure is not None:
                                if (not self._extruder_running and prev_pressure < 50.0 and current_pressure > 300.0):
                                    should_send = (
                                        self._last_start_email_at is None
                                        or (datetime.utcnow() - self._last_start_email_at).total_seconds()
                                        > self._start_stop_email_cooldown_seconds
                                    )
                                    if should_send:
                                        await notification_service.send_extruder_started_email()
                                        self._last_start_email_at = datetime.utcnow()
                                    self._extruder_running = True
                                # Shutdown detection: pressure > 300 -> < 50
                                elif self._extruder_running and prev_pressure > 300.0 and current_pressure < 50.0:
                                    should_send = (
                                        self._last_shutdown_email_at is None
                                        or (datetime.utcnow() - self._last_shutdown_email_at).total_seconds()
                                        > self._start_stop_email_cooldown_seconds
                                    )
                                    if should_send:
                                        await notification_service.send_extruder_shutdown_email()
                                        self._last_shutdown_email_at = datetime.utcnow()
                                    self._extruder_running = False
                    except Exception as alert_exc:
                        logger.error(
                            "Pressure alert / start-stop detection failed: %s",
                            alert_exc,
                            exc_info=True,
                        )
                    ...
                    # No new rows this cycle: still run pressure/start-stop check using latest window data
                    # so auto emails work even when MSSQL isn't updated every poll (e.g. data every few minutes).
                    if self._window:
                        readings, meta = self._compute_features()
                        current_pressure = readings.get("pressure")
                        current_rpm = readings.get("rpm")
                        if current_pressure is not None:
                            ...
                            try:
                                ...
                                logger.error(
                                    "Pressure alert (no new rows) failed: %s",
                                    alert_exc,
                                    exc_info=True,
                                )
                    else:
